...
### Definition
Spatial resolution refers to the dimensions of an image, specifically the number of [[1.6 Pixel Relationships and Distance|pixels]] it contains. It is typically expressed as the product of the image's width and height in pixels (e.g., `1920 x 1080` pixels).
...
-   **8-bit ([[1.3 Color Spaces (RGB, Grayscale, HSL)|Grayscale]]):** Each pixel has one of 256 possible shades of gray. `2^8 = 256` values (from 0 to 255).
-   **24-bit (True Color):** This is the standard for most color images. It uses 8 bits for each of the three [[1.3 Color Spaces (RGB, Grayscale, HSL)|color channels]] (Red, Green, and Blue).
...
| Keyword                 | Description                                                                                             |
| ----------------------- | ------------------------------------------------------------------------------------------------------- |
| [[Spatial Resolution]]  | The number of pixels in an image, typically expressed as width x height.                                |
| [[Color Depth]]         | The number of bits used to represent the color of a single pixel, determining the number of possible colors. |
| [[PPI]]                 | Pixels Per Inch; a measure of pixel density on a digital display.                                       |
| [[DPI]]                 | Dots Per Inch; a measure of dot density for printed images.                                             |
| [[Binary Image]]        | An image where each pixel can only be one of two colors (e.g., black or white).                         |
| [[True Color]]          | A 24-bit color depth that provides approximately 16.7 million colors.                                   |

---
tags: #introduction #resolution #color-depth #bpp