...
4.  **Gateway Issue:** The note correctly states that since B is not on the same subnet as A, it cannot use A's gateway. The final remark is crucial: _"La passerelle doit être dans même SR que l'hôte qui l'utilise"_ (The gateway must be in the same subnet as the host that uses it).
