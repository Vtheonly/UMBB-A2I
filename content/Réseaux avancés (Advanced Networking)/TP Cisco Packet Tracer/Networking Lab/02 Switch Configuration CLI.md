...
stateDiagram-v2
    direction LR
    [*] --> UserExec
    UserExec --> PrivilegedExec : type "enable"
    PrivilegedExec --> UserExec : type "disable"
    PrivilegedExec --> GlobalConfig : type "config terminal"
    GlobalConfig --> PrivilegedExec : type "exit"
    GlobalConfig --> InterfaceConfig : type "interface fa0/1"
    InterfaceConfig --> GlobalConfig : type "exit"

    state UserExec {
        note: Prompt: Switch>
        note: Limited view. Can only ping or show basic status.
    }
    state PrivilegedExec {
        note: Prompt: Switch#
        note: "Root" mode. Can show full config, restart, save.
    }
    state GlobalConfig {
        note: Prompt: Switch(config)#
        note: Mode to CHANGE settings (hostname, passwords).
    }
```
...
_(Notice the prompt changed from Switch to S1)._
...
_Note: If you use both, `secret` overrides `password`._
...
_(The `#` symbol acts as the start and stop delimiter)._
...
_Shortcut:_ `write` (or just `wr`) works on older IOS versions.
